...
describe('CR-5a: zustand persist 設定', () => {
    ...
    it('persistMigrate: version=3 データは uniqueDiceConfig 補完で通過する（基本動作）', () => {
        ...
        // CR-SA-17-E1 / 2026-06-06: enablePhaseConfig もデフォルト補完されて通過
        // CR-SA-20-E1 / 2026-06-11: enableFormationDice もデフォルト補完されて通過
...
describe('Bundle-1 / D-5 / 2026-05-09 houseRules 拡張フィールドの初期値検証', () => {
    ...
    // CR-SA-17-E1 / 2026-06-06: フェーズ構成変更の初期値検証（houserule-features.md §7.2 デフォルト）。
    it('CR-SA-17-E1 初期値: enablePhaseConfig=false / startPhaseCount=1 / endPhaseCount=1 / pacePosition=Start', () => {
        const config = useRaceStore.getState().config;
        expect(config.houseRules.enablePhaseConfig).toBe(false);
        expect(config.startPhaseCount).toBe(1);
        expect(config.endPhaseCount).toBe(1);
        expect(config.pacePosition).toBe('Start');
    });

    // CR-SA-20-E1 / 2026-06-11: 隊列〔バ群〕ダイスの初期値検証（houserule-features.md §6.7 デフォルト false = OFF 透過）。
    // 初期 state と DEFAULT_HOUSE_RULES の双方が false で一致すること（新規セッションと旧データ補完で同じ初期値）。
    it('CR-SA-20-E1 初期値: enableFormationDice=false（初期 state / DEFAULT_HOUSE_RULES 一致）', () => {
        expect(useRaceStore.getState().config.houseRules.enableFormationDice).toBe(false);
        expect(DEFAULT_HOUSE_RULES.enableFormationDice).toBe(false);
    });
});
...
describe('Bundle-7 / P4-6 / 2026-05-10 persistMigrate (zod 検証 + デフォルト補完)', () => {
    ...
    it('(i) version=1 旧データ（enableExtendedUnique / effectValue 欠落）→ デフォルト補完 + 既存 3 フィールド維持', () => {
        ...
        // CR-SA-17-E1 / 2026-06-06: enablePhaseConfig 追加で 8 フィールド構造になる
        // CR-SA-20-E1 / 2026-06-11: enableFormationDice 追加で 9 フィールド構造になる
        expect(result.config.houseRules).toEqual({
            enableModifier: true,
            enableSpecialStrategy: true,
            enableCompositeUnique: false,
            ...
            enablePhaseConfig: false, // CR-SA-17-E1 デフォルト補完
    ...
    it('(ii) houseRules 自体が undefined → デフォルト 6 フィールドで補完', () => {
        ...
        // CR-SA-17-E1 / 2026-06-06: enablePhaseConfig 追加で 8 フィールド構造
        // CR-SA-20-E1 / 2026-06-11: enableFormationDice 追加で 9 フィールド構造
        expect(result.config.houseRules).toEqual({
            enableModifier: false,
            enableSpecialStrategy: false,
            enableCompositeUnique: false,
...
describe('useRaceStore.persistMigrate - Bundle-8-T1 / v2→v3 マイグレーション', () => {
    ...
    it('(ii) v3 データ → uniqueDiceConfig 補完で検証成功通過', () => {
        ...
        // CR-SA-17-E1 / 2026-06-06: enablePhaseConfig もデフォルト補完される
        // CR-SA-20-E1 / 2026-06-11: enableFormationDice もデフォルト補完される
        expect(result.config.houseRules).toEqual({
    ...
    // (vi) CR-SA-17-E1 / 2026-06-06 ★最重要: v6 旧データ（houseRules.enablePhaseConfig 欠落 +
    // config.startPhaseCount / endPhaseCount / pacePosition 欠落）→ persistMigrate で version 7 へ補完される
    // （既存ユーザーデータ保護）。enablePhaseConfig は `...DEFAULT_HOUSE_RULES` 浅いマージで透過補完、
    // config 直下 3 フィールドは明示補完。旧データのカスタム値（他フラグ・uniqueDiceConfig・midPhaseCount 等）は保持。
    it('(vi) v6 旧データ → enablePhaseConfig=false + config フェーズ構成 3 フィールドのデフォルト補完 + カスタム値保持', () => {
        const v6Persisted = {
            config: {
                midPhaseCount: 3, // カスタム値（保持されること）
                fullGateSize: 16, // カスタム値（保持されること）
                // startPhaseCount / endPhaseCount / pacePosition は v6 では存在しない（欠落 = 補完対象）
                houseRules: {
                    enableModifier: true, // カスタム値（保持されること）
                    enableSpecialStrategy: false,
                    enableCompositeUnique: false,
                    enableExtendedUnique: true,
                    enableBondSkill: false,
                    effectValue: 22, // カスタム値（保持されること）
                    // v6 = 7 キー揃った uniqueDiceConfig（Stability カスタム値）
                    uniqueDiceConfig: {
                        ...DEFAULT_UNIQUE_DICE_CONFIG,
                        Stability: { fixValue: 9, diceStr: '1d11' },
                    },
                    // enablePhaseConfig は v6 では存在しない（欠落 = 補完対象）
                },
            },
            participants: [],
            currentPhaseId: 'Mid1',
            ...
            strategies: [],
            gateAssignments: null,
            appliedPresetName: 'マイ設定',
            isPresetDirty: false,
            uiState: { scene: 'race' },
        } as unknown as PersistedRaceState;

        const result = persistMigrate(v6Persisted, 6);

        // houseRules.enablePhaseConfig はデフォルト false で補完される
        expect(result.config.houseRules.enablePhaseConfig).toBe(false);
        // config 直下 3 フィールドがデフォルト補完される（序盤 1 / 終盤 1 / ペース = 序盤直後 'Start'）
        expect(result.config.startPhaseCount).toBe(1);
        expect(result.config.endPhaseCount).toBe(1);
        expect(result.config.pacePosition).toBe('Start');
        // 旧データのカスタム値は保持される
        expect(result.config.midPhaseCount).toBe(3);
        expect(result.config.fullGateSize).toBe(16);
        expect(result.config.houseRules.enableModifier).toBe(true);
        expect(result.config.houseRules.enableExtendedUnique).toBe(true);
        expect(result.config.houseRules.effectValue).toBe(22);
        expect(result.config.houseRules.uniqueDiceConfig.Stability).toEqual({ fixValue: 9, diceStr: '1d11' });
        // 他フィールドの保持（CR-SA-16-E1 appliedPresetName など）
        expect(result.appliedPresetName).toBe('マイ設定');
    });

    // (vii) CR-SA-17-E1 / 2026-06-06: config 自体が存在しない超旧データも全フィールドがデフォルト補完で復元される
    // （fallback 経路 + 明示補完の二重保証。検証失敗 → 全リセットが起きないこと）。
    it('(vii) config 欠落の超旧データ → フェーズ構成 3 フィールド + enablePhaseConfig 全デフォルト補完', () => {
        const noConfig = {
            participants: [],
            currentPhaseId: 'setup',
            paceResult: { face: null, label: null },
        ...
        const result = persistMigrate(noConfig, 1);

        expect(result.config.startPhaseCount).toBe(1);
        expect(result.config.endPhaseCount).toBe(1);
        expect(result.config.pacePosition).toBe('Start');
        expect(result.config.houseRules.enablePhaseConfig).toBe(false);
        // CR-SA-20-E1 / 2026-06-11: enableFormationDice もデフォルト補完される（DEFAULT_HOUSE_RULES 9 フィールド）
        expect(result.config.houseRules.enableFormationDice).toBe(false);
        // houseRules 全体がデフォルト補完（既存挙動）
        expect(result.config.houseRules).toEqual(DEFAULT_HOUSE_RULES);
    });

    // (viii) CR-SA-20-E1 / 2026-06-11 ★最重要: v7 旧データ（houseRules.enableFormationDice 欠落）→
    // persistMigrate で version 8 へ補完される（既存ユーザーデータ保護）。enableFormationDice は
    // `...DEFAULT_HOUSE_RULES` 浅いマージで透過補完（enablePhaseConfig と同方式）。
    // 旧データのカスタム値（他フラグ・uniqueDiceConfig・フェーズ構成・midPhaseCount 等）は保持。
    it('(viii) v7 旧データ → enableFormationDice=false のデフォルト補完 + カスタム値保持', () => {
        const v7Persisted = {
            config: {
                midPhaseCount: 2, // カスタム値（保持されること）
                fullGateSize: 14, // カスタム値（保持されること）
                // v7 で導入済のフェーズ構成フィールド（カスタム値、保持されること）
                startPhaseCount: 2,
                endPhaseCount: 3,
                pacePosition: 'Mid1',
                houseRules: {
                    ...
                    enableExtendedUnique: true,
                    enableBondSkill: false,
                    effectValue: 33, // カスタム値（保持されること）
                    // v7 = 7 キー揃った uniqueDiceConfig（Gamble カスタム値）
                    uniqueDiceConfig: {
                        ...DEFAULT_UNIQUE_DICE_CONFIG,
                        Gamble: { fixValue: -3, diceStr: '1d30' },
                    },
                    enablePhaseConfig: true, // v7 カスタム値（保持されること）
                    // enableFormationDice は v7 では存在しない（欠落 = 補完対象）
                },
            },
            participants: [],
            currentPhaseId: 'Mid1',
            paceResult: { face: 3, label: 'Middle' },
            strategies: [],
            gateAssignments: null,
            appliedPresetName: 'マイ設定v7',
            isPresetDirty: true,
            uiState: { scene: 'race' },
        } as unknown as PersistedRaceState;

        const result = persistMigrate(v7Persisted, 7);

        // houseRules.enableFormationDice はデフォルト false で補完される
        expect(result.config.houseRules.enableFormationDice).toBe(false);
        // 旧データのカスタム値は保持される
        expect(result.config.midPhaseCount).toBe(2);
        ...
        expect(result.config.houseRules.enableModifier).toBe(true);
        expect(result.config.houseRules.enableExtendedUnique).toBe(true);
        expect(result.config.houseRules.effectValue).toBe(33);
        expect(result.config.houseRules.enablePhaseConfig).toBe(true);
        expect(result.config.houseRules.uniqueDiceConfig.Gamble).toEqual({ fixValue: -3, diceStr: '1d30' });
        // 他フィールドの保持（CR-SA-16-E1 appliedPresetName など）
        expect(result.appliedPresetName).toBe('マイ設定v7');
        expect(result.isPresetDirty).toBe(true);
    });

    // CR-SA-21+22-E1 / 2026-07-06 ★最重要: v9 旧データ（houseRules.enableNoUniqueSkill /
    // customUniqueSkills 欠落）→ persistMigrate で version 10 へ補完される（既存ユーザーデータ保護）。
    // 補完は DEFAULT_HOUSE_RULES 拡張の透過マージ + zod .default() 経路の二重で担保。
    // 旧データのカスタム値（他フラグ・uniqueDiceConfig・フェーズ構成・formationResult 等）は保持。
    it('(ix) v9 旧データ → enableNoUniqueSkill=false / customUniqueSkills=[] のデフォルト補完 + カスタム値保持', () => {
        const v9Persisted = {
            config: {
                midPhaseCount: 2, // カスタム値（保持されること）
                fullGateSize: 14, // カスタム値（保持されること）
                ...
                houseRules: {
                    ...
                    enableExtendedUnique: true,
                    enableBondSkill: false,
                    effectValue: 42, // カスタム値（保持されること）
                    uniqueDiceConfig: {
                        ...DEFAULT_UNIQUE_DICE_CONFIG,
                        Stability: { fixValue: 6, diceStr: '1d12' }, // カスタム値
                    },
                    enablePhaseConfig: true, // v9 カスタム値（保持されること）
                    enableFormationDice: true, // v9 カスタム値（保持されること）
                    // enableNoUniqueSkill は v9 では存在しない（欠落 = 補完対象）
                    // customUniqueSkills は v9 では存在しない（欠落 = 補完対象）
                },
            },
            participants: [],
            currentPhaseId: 'Mid1',
            paceResult: { face: 7, label: 'High' },
            // v9 で導入された formationResult も保持
            formationResult: { face: 5, label: '普通' },
            strategies: [],
            gateAssignments: null,
            appliedPresetName: 'マイ設定v9',
            isPresetDirty: true,
            uiState: { scene: 'race' },
        } as unknown as PersistedRaceState;

        const result = persistMigrate(v9Persisted, 9);

        // 新 2 フィールドはデフォルト補完される
        ...
        // v9 導入の formationResult / paceResult も保持
        expect(result.formationResult).toEqual({ face: 5, label: '普通' });
        expect(result.paceResult).toEqual({ face: 7, label: 'High' });
        // 他フィールドの保持
        expect(result.appliedPresetName).toBe('マイ設定v9');
        expect(result.isPresetDirty).toBe(true);
    });

    // CR-SA-21+22-E1 / 2026-07-06: v9 データにカスタム固有スキル登録済のケースを想定した
    // 「疑似 v10 相当」データ透過保持テスト（正常経路の再現）。superRefine 通過確認も兼ねる。
    it('(x) customUniqueSkills を含む v10 相当データ → 全フィールド透過保持（マイグレ後も同値）', () => {
        const entries = [
            { id: 'cs-1', name: '先行特化', fixValue: 5, diceStr: '1d10' },
            { id: 'cs-2', name: '差し加速', fixValue: -5, diceStr: '1d20' },
        ];
        const v10Persisted = {
            config: {
                midPhaseCount: 1,
                ...
                houseRules: {
                    ...
                    enablePhaseConfig: false,
                    enableFormationDice: false,
                    enableNoUniqueSkill: true, // 明示 ON
                    customUniqueSkills: entries, // 明示登録
                },
            },
            participants: [],
            currentPhaseId: 'setup',
...
describe('CR-SA-16-E1 / 2026-05-15 appliedPresetName + isPresetDirty', () => {
    ...
    it('(S19) 初期 state: houseRules.enableNoUniqueSkill=false / customUniqueSkills=[]', () => {
        ...
        expect(DEFAULT_HOUSE_RULES.enableNoUniqueSkill).toBe(false);
        expect(DEFAULT_HOUSE_RULES.customUniqueSkills).toEqual([]);
    });

    // CR-SA-21+22-E1 / 2026-07-06: persistPartialize に新 2 フィールドが含まれる（config 経由）
    it('(S20) persistPartialize: config.houseRules.enableNoUniqueSkill / customUniqueSkills が含まれる', () => {
        const state = useRaceStore.getState();
        const partialized = persistPartialize(state);
        expect(partialized.config.houseRules.enableNoUniqueSkill).toBe(false);
        expect(partialized.config.houseRules.customUniqueSkills).toEqual([]);
    });

    // CR-SA-21+22-E1 / 2026-07-06: updateHouseRules で 2 フィールドを上書きできる
    it('(S21) updateHouseRules: customUniqueSkills 上書き + enableNoUniqueSkill トグル', () => {
        useRaceStore.getState().updateHouseRules({ enableNoUniqueSkill: true });
        expect(useRaceStore.getState().config.houseRules.enableNoUniqueSkill).toBe(true);

        const entries = [{ id: 'x1', name: '先行特化', fixValue: 5, diceStr: '1d10' }];
        useRaceStore.getState().updateHouseRules({ customUniqueSkills: entries });
        expect(useRaceStore.getState().config.houseRules.customUniqueSkills).toEqual(entries);
...
describe('useRaceStore - CR-SA-16-Followup-reset-houserules / 2026-06-06 resetHouseRules action', () => {
    ...
    // (R5) CR-SA-17-E1 / 2026-06-06: enablePhaseConfig は reset 対象（false 復帰）、
    // config 直下のフェーズ構成（startPhaseCount / endPhaseCount / pacePosition）は reset 非対象（保持）。
    // houserule-features.md §7.8 + modal-houserule.md §5 初期化対象範囲の整合確認。
    it('(R5) enablePhaseConfig は reset で false 復帰 / startPhaseCount 等 config 値は保持', () => {
        // enablePhaseConfig を ON にし、config のフェーズ構成をカスタム値に設定
        useRaceStore.getState().updateHouseRules({ enablePhaseConfig: true });
        useRaceStore.setState((state) => ({
            config: {
                ...state.config,
        ...
        expect(useRaceStore.getState().config.houseRules.enablePhaseConfig).toBe(true);

        useRaceStore.getState().resetHouseRules();

        const state = useRaceStore.getState();
        // enablePhaseConfig はハウスルール = reset 対象 → false 復帰（DEFAULT_HOUSE_RULES 由来）
        expect(state.config.houseRules.enablePhaseConfig).toBe(false);
        // startPhaseCount / endPhaseCount / pacePosition は config 個別設定 = reset 非対象 → 保持
        expect(state.config.startPhaseCount).toBe(3);
        expect(state.config.endPhaseCount).toBe(2);
        expect(state.config.pacePosition).toBe('Mid1');
    });

    // (R6) CR-SA-20-E1 / 2026-06-11: enableFormationDice はハウスルールトグル = reset 対象（false 復帰）。
    // DEFAULT_HOUSE_RULES への置換（resetHouseRules）で自動的に効くことの担保（houserule-features.md §6.7）。
    it('(R6) enableFormationDice は reset で false 復帰', () => {
        useRaceStore.getState().updateHouseRules({ enableFormationDice: true });
        expect(useRaceStore.getState().config.houseRules.enableFormationDice).toBe(true);
