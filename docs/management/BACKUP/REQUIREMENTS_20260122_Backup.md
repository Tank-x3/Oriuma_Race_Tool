...
2.  **コピー用テキスト生成 (Text Generation)**
    * **確認用リスト:** `[エントリー番号]. [名前] ([脚質] / [固有タイプ] / [発動位置])` の形式。
    * **ダイスリスト:** `[名前] (全角スペース) dice1d100=` の形式。
...
│   │       └── implementations/
│   │           └── AnimanParser.ts # Default parser logic
