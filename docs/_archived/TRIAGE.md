...
- **開始日:** 2026-04-22
- **現在のフェーズ:** **Step 4/5 完了 — §6-B 順序付け・ロール割付 + §7 復帰手順 記入完了。§1 解除条件 5 項目すべてクリア。ユーザー明示宣言による TRIAGE 解除待ち**
- **最終更新:** 2026-04-24（Wave 改番: 旧 Wave 5 → Wave 0「解除処理」へ。Wave 番号と実行順を一致させ、§0/§6-B/§7 のフロー整合性を修正）

### 次セッション再開ポイント（2026-04-24 時点、TRIAGE 解除宣言待ち）

**§6-B + §7 完了 + フロー整合性修正済。次は §6-B Wave 0〜4 計画と §7 をユーザーがレビューし、明示宣言による解除手続きへ。**

1. CLAUDE.md 冒頭の TRIAGE ブロックが残っていることを確認（未解除）
2. 本 TRIAGE.md §6-B の Wave 0（解除処理）〜 Wave 4（解除後タスク）計画と §7 復帰手順をユーザーがレビュー
3. レビューで修正要望があれば本 TRIAGE.md を更新（TRIAGE 内の許可作業）
4. **ユーザーが「TRIAGE 解除」を明示宣言**
5. **Wave 0（DOC-1f + TRI-4）実施**: CLAUDE.md 冒頭 TRIAGE ブロック削除 + TRIAGE.md アーカイブ → **通常ワークフロー復帰**
6. 復帰後の第 1 セッションは **PM ロールで Wave 1 REPO 系** から着手（§7 参照）

## 1. TRIAGE 解除条件

以下を全て満たし、**ユーザーが明示的に「TRIAGE 解除」と宣言した**ときのみ、本モードを解除してよい。

- [x] Step 1: ユーザーが「やるべきと認識していること」の自由記述が一旦完了している（§3 記入完了、2026-04-22）
- [x] Step 2: Claude による整理・構造化（項目化・分類・優先度付け）が完了し、ユーザーがレビュー済み（§4 初稿記入 + 曖昧点 (a)〜(h) 全クローズ、2026-04-23）
- [x] Step 3: 既存ドキュメント（STATE.md / BOARD.md / CODE_REVIEW_BOARD.md / ROADMAP.md 等）とのクロスリファレンスが完了し、齟齬・重複・漏れが洗い出されている（§5-1〜§5-4 全 4 ブロック完了、2026-04-23）
- [x] Step 4: 整理結果として「今やるべきこと」の確定リストが本ドキュメントに記載され、既存ドキュメント側にも必要な反映（またはポインタ）が完了している（§6-A 全項目リスト + §6-B 順序付け・ロール割付完了、未決論点 (l)(m)(n)(e) 全件確定、2026-04-23）
- [x] Step 5: 通常ワークフロー復帰手順（どのロールで何から着手するか）が明確になっている（§7 復帰手順記入完了、Wave 1〜5 想定 6 セッション、2026-04-23）
...
| **DOC-1f** | TRIAGE 解除時に CLAUDE.md 構成への `docs/handover/TRIAGE.md` 記載方針（追記 or 削除）を確定 | §5-4 B群 | DOC-1 是正候補 #6。**Wave 0（TRIAGE 解除処理）内で TRI-4 と同時実施** |
...
| ID | タスク | 備考 |
|----|-------|------|
| **TRI-3** | Step 5（§7）の復帰手順確定 | 2026-04-23 記入完了。Wave 0 の前処理として本セッション内完了済 |
| **TRI-4** | ユーザー明示宣言による TRIAGE 解除 → CLAUDE.md 冒頭 TRIAGE ブロック削除 + `docs/handover/TRIAGE.md` のアーカイブ or 削除 | **Wave 0 内で DOC-1f と同時実施（Wave 1〜4 に先行）** |
...
**前提（6-A までに確定済の既決方針）:**
- (i) 優先順: **REPO 群 → CR-38 → DEV-1 → その他（DOC 系）**。CR-SA-2 は Phase 4 前提のため据え置き
...
| 対象領域 | 担当ロール |
|---------|-----------|
| `guidelines/*.md`, `CLAUDE.md`, `.gitignore`, git 運用 | PM |
| `docs/management/BOARD.md`, `docs/ROADMAP.md` | PM |
| `docs/specs/**`, `docs/REQUIREMENTS.md`, `docs/APP_FLOW.md` | SA |
| `src/**` 実装 | Engineer |
| 運用判断（Docs 構造・push タイミング等） | PM |
| 仕様判断（UI/ロジック/アーキテクチャ） | SA |

#### Wave 0: TRIAGE 解除処理（最優先・TRIAGE 内部手続き） — PM + User

**前提:** §1 解除条件 5 項目すべてクリア済、ユーザーレビュー完了後に実施。Wave 1〜4 は本 Wave 完了（= TRIAGE ブロック削除）後にのみ着手可能。

| 順序 | ID | タスク要約 | ロール |
|------|----|----------|-------|
| W0-1 | **TRI-3** | 本 TRIAGE.md §7 復帰手順の記入完了 | 2026-04-23 記入済（Wave 0 の前処理） |
| W0-2 | **ユーザー明示宣言** | ユーザーが「TRIAGE 解除」と明示宣言 | User |
| W0-3 | **TRI-4 (part 1)** | CLAUDE.md 冒頭の TRIAGE モード宣言ブロックを削除 → **この時点で通常ワークフロー復帰が技術的に完了** | PM |
| W0-4 | **DOC-1f** | CLAUDE.md §43 ディレクトリ構成の `docs/handover/` 記述から `TRIAGE.md` 言及を削除（CLAUDE.md 構成への記載方針 = 削除 を確定） | PM |
| W0-5 | **TRI-4 (part 2)** | `docs/handover/TRIAGE.md` を `docs/_archived/TRIAGE.md` へ移動（履歴保持、削除はしない） | PM |
| W0-6 | — | Wave 0 全体をコミット（push は次の PM Step 3 Verification で実施 = REPO-3 ルール適用前なので手動 push または Wave 1 REPO-2 コミットと一緒に push） | PM |

**重要:** Wave 0 は「TRIAGE 内部で許可される作業（TRIAGE.md の更新および CLAUDE.md ブロック削除に伴う付随的な CLAUDE.md / TRIAGE.md 編集）」として例外的に実施される。Wave 1 以降は Wave 0 完了後の通常ワークフロー上で行う。

#### Wave 1: REPO 整備（最優先・解除後の第 1 セッション） — PM 単独・1 セッション想定

| 順序 | ID | タスク要約 | ロール |
|------|----|----------|-------|
| W1-1 | **REPO-0** | 未コミット残滓（`M docs/CODE_REVIEW_BOARD.md` +1924 行 / `?? docs/FINDINGS_CONSOLIDATED.md`）を `git add` → コミット（push はしない） | PM |
| W1-2 | **REPO-1** | `.gitignore` に `docs/` + `work_logs/` + `guidelines/` + `CLAUDE.md` を追加 → コミット | PM |
| W1-3 | **REPO-2** | `git rm -r --cached` で tracked 解除（対象: 除外対象 ∩ tracked）→ コミット → **push**（force-push 禁止、履歴残置） | PM |
| W1-4 | **REPO-3** | `guidelines/pm.md` Step 3 Verification に「GitHub push を規定値」として明記、`guidelines/engineer.md` Step 3b に「push は PM 側」を注記（REPO-1 後はローカルのみの変更になるが意図通り） | PM |

#### Wave 2: CR-38 対応（致命的・進行ブロック解消） — SA → Engineer

| 順序 | ID | タスク要約 | ロール |
|------|----|----------|-------|
| W2-1 | **CR-38-SA** | `docs/specs/logic/basic-rules.md` §6「途中修正運用ルール」に「固有タイプ修正時の history 挙動（自動クリア / 手動リセット / 警告表示など）」を追記し実装方針提示。**DOC-3a の basic-rules.md §6 部分は本タスクに吸収** | SA |
| W2-2 | **CR-38-E** | CR-38 実装（SA 方針に基づく）。完了後 `work_logs/` に記録 | Engineer |

#### Wave 3: DEV-1 対応（ダークモード UX バグ） — Engineer（Wave 2-E と同セッションで合流可）

| 順序 | ID | タスク要約 | ロール |
|------|----|----------|-------|
| W3-1 | **DEV-1a** | `src/components/scene/JudgmentScene.tsx`（Scene 4-A）のダークモード配色整備。基準は `GateScene.tsx`（dark: 33 件）。337 行中の背景 / 文字 / 枠線色全般 | Engineer |
| W3-2 | **DEV-1b** | `src/components/scene/ResultScene.tsx`（Scene 4-B、dark: 0 件）のダークモード配色整備。DEV-1a と連続で Scene 4 フロー一体修正 | Engineer |

#### Wave 4: DOC 整備（その他） — PM 1 セッション + SA 1 セッション想定

| 順序 | ID | タスク要約 | ロール |
|------|----|----------|-------|
| W4-1 | **DOC-2** | Docs リファクタ再開 or やり直し判断。**TRIAGE 整理結果を踏まえ「構造維持 + 問題箇所是正」を原則として確定**（破棄案を却下した時点で実質確定済、Wave 4 冒頭で明文化のみ） | PM（判断） |
| W4-2 | **DOC-1a** | CLAUDE.md §43 ディレクトリ構成に `docs/ideas/BOARD.md` / `docs/ideas/REJECTED.md` / `docs/ARCHITECT_FEEDBACK.md` / `docs/management/DEPLOY_GUIDE.md` を追記 | PM |
| W4-3 | **DOC-1b** | CLAUDE.md §28「移行・セットアップ」を削除 or 「移行完了済」注記化 | PM |
| W4-4 | **DOC-1c** | `guidelines/engineer.md` Capabilities の `gas_src/` 言及を削除 | PM |
| W4-5 | **DOC-1d** | `guidelines/system-architect.md` の `PROJECT_OVERVIEW.md` 言及を削除 or 存在確認注記化 | PM |
| W4-6 | **DOC-1e** | 4 ガイドラインに「特別作業ドキュメント（CLAUDE.md §87 参照）は通常ロールセッションで編集しない」旨を追記 | PM |
| W4-7 | **DOC-4** | `work_logs/` を「正式な作業記録・コードベース情報整理のベース」と位置づける旨を `guidelines/*.md` / `CLAUDE.md` に明記 | PM |
| W4-8 | **DOC-3c** | `docs/management/BOARD.md` CR-10 スコープ記述に「Scene 2 バリデーションメッセージ文言（`REVIEW_20260121_Scene2.md` 指摘）の現状確認」を追記し DOC-3c クローズ | PM |
| W4-9 | **DOC-3a** | `docs/specs/architecture/parser-system.md` / `tech-stack.md` を実態反映更新（Zustand 採用 / 平坦ディレクトリ / 3 種 Parser コンテキスト / EmojiParser 減算仕様）。**CR-SA-1 駆動**（basic-rules.md §6 は W2-1 で処理済） | SA |
| W4-10 | **DOC-3b** | `parser-system.md` に `ParsedLine.isSubtractive` 設計注記（「現設計は diceResult 負数化で吸収、将来の検算ロジック拡張時に再確認」）。DOC-3a と同一セッションで実施 | SA |
| W4-11 | **DOC-3d** | `docs/REQUIREMENTS.md` の `(N)` / チェックサム仕様の反映状態を目視確認（記録上は対応完了済） | SA |

#### Wave 間の依存関係

- **Wave 0 が最優先（TRIAGE 解除処理）**。Wave 1〜4 はすべて Wave 0 完了後にのみ着手可能
- **Wave 1 は Wave 2〜4 すべてに先行**。GitHub 露出最小化とローカル専用化を最初に確立
- **Wave 2-SA → Wave 2-E** は直列（SA 仕様確定後に実装）
- **Wave 3 は Wave 2-E と合流可**（Engineer セッション内で順に実施）
- **Wave 4 は Wave 3 完了後**（(i) 優先順厳守）。PM 側の余力があれば Wave 4 PM 管轄項目の一部を Wave 2-SA と並行で先行させる余地あり

#### 想定セッション数（最小）

- Wave 0（TRIAGE 解除処理）: 本セッションまたは次の短いセッション内で実施（独立セッションとしてカウントしないか、ごく短い 1 回）
- PM セッション: 3 回（Wave 1 / Wave 4 PM 管轄 / ※ Wave 0 は PM 相当作業だが TRIAGE 内手続きのため別カウント）
- SA セッション: 2 回（Wave 2-SA / Wave 4 SA 管轄）
- Engineer セッション: 1 回（Wave 2-E + Wave 3 合流）
- 合計: **Wave 0 + 6 セッション**（粒度により増減）

Wave 0 完了時点で TRIAGE モード終了（CLAUDE.md 冒頭ブロック削除）。以降は通常ワークフローで Wave 1〜4 を順次実施し、完了後に Phase 3.5 残（BOARD.md の High CR-3〜CR-8 等）へ復帰。

## 7. 通常ワークフロー復帰手順（Step 5）

### Wave 0: TRIAGE 解除処理（TRIAGE モード内の最終手続き）

ユーザーが「TRIAGE 解除」を明示宣言したセッション内（または直後の短いセッション）で、現行セッションの Claude が PM 相当として実施。Wave 0 中は CLAUDE.md と TRIAGE.md の編集のみ例外的に許可される。

**手順（詳細）:**

1. CLAUDE.md 冒頭の TRIAGE モード宣言ブロック（`> ## ⚠️ TRIAGE モード中` から `> セッション開始時は...` まで）を削除（W0-3: TRI-4 part 1）
2. CLAUDE.md §43 ディレクトリ構成の `docs/handover/` 記述から `TRIAGE.md` 言及を削除（W0-4: DOC-1f）
3. `docs/handover/TRIAGE.md` を `docs/_archived/TRIAGE.md` へ移動（履歴として保持、削除はしない）（W0-5: TRI-4 part 2）
4. Wave 0 全体をまとめてコミット（push タイミングは W1-3 REPO-2 のコミット・push と合流させて 1 回にまとめる案を推奨）

### 解除直後の第 1 セッション（通常ワークフロー復帰後）

**ロール: PM**。ユーザーが新規セッションで「PM ロールで作業開始」と宣言して開始。

**初動アクション:**

1. `docs/handover/STATE.md` を読みプロジェクト状態把握（エスカレーション 3 件のうち CR-38 は Wave 2 で対応予定、CR-SA-1 は Wave 4 DOC-3a で対応予定、CR-SA-2 は Phase 4 前提で据え置きと認識）
2. アーカイブされた `docs/_archived/TRIAGE.md` §6-B Wave 1 計画を再確認
3. `docs/management/BOARD.md` に Wave 1〜4 の各項目を **新 Issue として追加登録**（既存 Issue との重複を避け、TRIAGE 由来と明記）
4. **Wave 1 REPO 系を実施:**
   - REPO-0: 未コミット残滓コミット
   - REPO-1: `.gitignore` 更新コミット
   - REPO-2: `git rm -r --cached` → コミット → push（Wave 0 の成果物があれば同梱 push）
   - REPO-3: `guidelines/pm.md` Step 3 + `guidelines/engineer.md` Step 3b にルール追記
5. Step 3 Verification: Wave 1 コミット群と GitHub push の完了を確認
6. 次セッション引継ぎ: `docs/handover/STATE.md` の「次回アクション」を **SA ロール / CR-38-SA** に更新

### 解除後のロール起動順（想定）

| セッション | ロール | 内容 | 前提 |
|-----------|-------|------|------|
| **第 0（TRIAGE 内）** | **現行 Claude (PM 相当)** | **Wave 0 解除処理（ユーザー明示宣言 → CLAUDE.md ブロック削除 + DOC-1f + TRIAGE.md アーカイブ）** | **§1 解除条件 5 項目クリア + ユーザー明示宣言** |
| 第 1 | PM | Wave 1 REPO 系（REPO-0〜3） | Wave 0 完了（= TRIAGE 解除済） |
| 第 2 | SA | Wave 2-SA CR-38 仕様追記 + 実装方針 | PM#1 完了 |
| 第 3 | Engineer | Wave 2-E CR-38 実装 + Wave 3 DEV-1a/b | SA#1 完了 |
| 第 4 | PM | Wave 4 PM 管轄（DOC-2 判断 → DOC-1a〜e → DOC-4 → DOC-3c） | Engineer#1 完了 |
| 第 5 | SA | Wave 4 SA 管轄（DOC-3a → DOC-3b → DOC-3d） | PM#2 完了 |

第 0 セッション（Wave 0）完了時点で **TRIAGE モード終了**。第 5 セッション（Wave 4 SA）完了時点で TRIAGE 由来タスク全消化 → 通常ワークフローで Phase 3.5 残（BOARD.md の High CR-3〜CR-8 等）へ復帰。

### ロール切替のトリガー

- 各ロール完了時に **STATE.md の「次回アクション」セクションを更新**し、次のロール／内容を明示する。ユーザーはこれを見て次セッションのロールを決定する
- AI 側からセッション切替を促すことは CLAUDE.md §23 / 4 ガイドラインの Session Management に従い**禁止**
- ユーザーが「TRIAGE 解除」を明示宣言する前に Wave 1 以降の作業（REPO 整備や Phase 3.5 残の実装など）に着手しない
...
- 2026-04-24: §1 解除条件 5 項目にチェック付与（全クリア宣言）。ユーザー指摘により §0 / §6-B / §7 のフロー整合性矛盾を修正: 旧 Wave 5（解除手続き）→ **Wave 0（TRIAGE 解除処理）** に改番。Wave 0 を Wave 1〜4 に先行させ、「解除処理 → 通常ワークフロー復帰 → Wave 1〜4 実施」の順序を明確化。§7 ロール起動順の表に「第 0（TRIAGE 内）」を追加、Wave 番号と実行順の逆転を解消
- 2026-04-23: §6-B 順序付け・ロール割付を記入（Wave 1 REPO / Wave 2 CR-38 / Wave 3 DEV-1 / Wave 4 DOC / Wave 5 TRIAGE 解除手続き、想定 6 セッション）。§7 復帰手順を記入。§7 重複（2 箇所 "未着手"）を 1 つに集約。Step 4/5 完了、ユーザーレビュー → TRIAGE 解除宣言待ちへ
